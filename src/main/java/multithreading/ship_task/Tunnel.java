...
/**
 * Класс тунель, в него можно добавлять и извлекать корабли. Максимальная вместиомсть 5 кораблей.
 * */
    ...
    public synchronized boolean add(Ship element) {
        try {
            if(shipsCounter < maxShipsInTunel) {
                notifyAll();
                store.add(element);
                System.out.println(String.format("%s + The ship arrived in the tunnel: %s %s %s",
